...
/**
 * Skeleton怪物类
 * 继承自BaseMonster，实现Skeleton特有的属性和行为
 */
@regClass()
export class SkeletonMonster extends BaseMonster {
    
    // Skeleton的动画管理器引用
    private skeletonAnimationManager: SkeletonAnimationManager | null = null;
    ...
    protected initializeMonster(): void {
        // 根据等级设置Skeleton的基础属性
        this.monsterStats = this.calculateSkeletonStats();
    }
    ...
    protected getAnimationManagerComponent(): any {
        this.skeletonAnimationManager = this.owner.getComponent(SkeletonAnimationManager);
        return this.skeletonAnimationManager;
    }
